 ...
 * FORMAT DATA
 */
const topics = topicsData.map((topic) => `('${topic.type.replaceAll("'", "''")}', '${topic.name.replaceAll("'", "''")}')`)
...
writer.write("DELETE FROM \"public\".\"UserTopic\" WHERE \"topicId\" IN (SELECT \"id\" FROM \"public\".\"Topic\" WHERE \"type\" IS NULL);\n")
writer.write("DELETE FROM \"public\".\"Topic\" WHERE \"type\" IS NULL;\n")

writer.close()

console.log(`Referential Topics successfully updated (${topics.length} topics).`)
