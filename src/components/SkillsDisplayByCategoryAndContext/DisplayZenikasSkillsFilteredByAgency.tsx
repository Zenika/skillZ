...
type DisplayZenikasSkillsFilteredByAgencyProps = {
  category: string;
  agency: string;
};
export const DisplayZenikasSkillsFilteredByAgency = ({
  category,
  agency,
}: DisplayZenikasSkillsFilteredByAgencyProps) => {
    ...
    () =>
      setFilterByAgency(
        agency
          ? {
              name: "Agency",
              values: data?.Agency.map((agency) => agency.name) || [],
              selected: agency,
            }
          : undefined
      ),
  ...
  const sortedSkills =
    data?.Category[0]?.AgenciesAverageCurrentSkillsAndDesires?.map(
      (skill: AgenciesAverageCurrentSkillsAndDesires) => ({
        id: skill.skillId,
        name: skill.name,
  ...
  return (
    <CommonPage page={category} faded={modaleOpened} context="zenika">
      <PageWithSkillList
        context="zenika"
        category={category}
        add={false}
