...
type DisplayZenikasSkillsProps = {
  category: string;
};

export const DisplayZenikasSkills = ({
  category,
}: DisplayZenikasSkillsProps) => {
  ...
  const radarData = data?.Category[0]?.ZenikasAverageCurrentSkillsAndDesires.map(
    (skill) => ({
  ...
  const sortedSkills =
    data?.Category[0]?.ZenikasAverageCurrentSkillsAndDesires.map(
      (skill: ZenikasAverageCurrentSkillsAndDesires) => ({
        id: skill.skillId,
        name: skill.name,
