...
const AddSkillListSelector = ({
     ...
     * QUERIES
     */
    const { data } = useQuery<SearchSkillsVerifiedQuery>(
    ...
    return (
        <div
            className={`flex flex-col my-4 ${isDesktop ? 'overflow-y-auto' : ''}`}
        >
            {tutorialMode && (
                <Joyride
                        ...
                        {data?.Skill.length > 0 && (
                            <div className="flex flex-col justify-center px-2 py-4">
                                <span className="p-2 text-center font-bold">
                                    {t('skills.globalSkillSearch').replace(
                                        '%skill%',
                                        search
                                    )}
                                </span>
