...
const Circle = ({
  ...
  const filteredCircles = data.filter(
    (row) => 6 - Math.floor(row.x) === i && Math.floor(row.y) + 1 === k
  );
  if (filteredCircles.length <= 0) {
    return <></>;
  }
  const circle = filteredCircles.reduce((previous, current) => ({
    ...previous,
    weight: (previous.weight || 8) + 1,
    names: [...previous.names, ...current.names],
  }));
  const xDetail = -4 + Math.floor(Number((circle.x % 1).toFixed(1)) * 9);
  const yDetail = -4 + Math.floor(Number((circle.y % 1).toFixed(1)) * 13);
  return (
    <div
      className={`
          flex flex-col 
        ...
        xDetail < 0 ? "-" : ""
      }top-${Math.abs(xDetail)} ${yDetail < 0 ? "-" : ""}left-${Math.abs(
        yDetail
      )} rounded-full text-center text-xs pt-${(circle.weight || 10) - 8} h-${
        circle.weight || 8
      } w-${circle.weight || 8} gradient-${color}`} // (-4; -4) (5, 9)
    >
      {circle.names.join(",")}
    </div>
  );
...
const RadarCell = ({
  ...
  return (
    ...
    >
      <Circle i={i} k={k} data={data} color={color} />
    </div>
  );
};
...
const RadarRow = ({
  ...
  return (
    <div className="flex flex-row w-full h-1/5" key={`${i}`}>
      {oneToFive.map((k) => (
        <RadarCell i={i} data={data} k={k} color={color} />
      ))}
    </div>
  );
};
  ...
  return (
    ...
    >
      <div className="w-full h-4/5">
        ...
        >
          {oneToFive.map((i) => (
            <RadarRow i={i} data={data} color={color} />
          ))}
        </div>
      </div>
