...
module.exports = {
    async changeCommandValueForAction(command) {
        ...
        for (let i = 0; i < allSkills.Skill.length; i++) {
            if (
                command.toUpperCase() === allSkills.Skill[i].name.toUpperCase()
            ) {
    ...
    actionNoteSkill(app) {
        app.action('noteSkill', async ({ ack, body, context }) => {
            ...
            if (
                ...
                newSkill = false;
                skillLastDesireValue =
                    response.User[0].UserSkillDesires[0].desireLevel;
                skillLastSkillValue =
                    response.User[0].UserSkillDesires[0].skillLevel;
            }
            try {
